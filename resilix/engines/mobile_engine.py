"""ResiliX Mobile Backend resilience testing engine.

Adapted from the legacy ``scripts/mobile_ddos.py`` Firebase/FCM/APNs
capabilities but bounded and safe. Tests mobile backend endpoints
...
class MobileTestEngine(TestEngine):
    """Bounded mobile backend resilience testing engine."""

    def __init__(self, config: TestConfig,
                 logger: Optional[StructuredLogger] = None) -> None:
        super().__init__(config, logger)
        self.capabilities = type(self.capabilities)(
            name="Mobile", supports_baseline=True, supports_recovery=True,
    ...
    def prepare(self) -> None:
        self._logger.info("info", "Mobile engine prepared",
    ...
    def _run_phase(self, phase) -> None:
        ...
        for i in range(min(phase.concurrency,
            ...
            w = threading.Thread(target=self._mobile_worker,
    ...
    def _mobile_worker(self, wid, phase, phase_stop) -> None:
        while not self._stop_flag and not phase_stop.is_set() and \
            ...
            try:
                ...
                resp.read()
                self._collector.record_success(0.0, resp.status)
            except Exception:
                self._collector.record_failure("mobile_error")
            finally:
                self._gate.release()
    ...
    def as_dict(self) -> Dict[str, Any]:
        return {"type": "Mobile", "target": str(self.config.target),
